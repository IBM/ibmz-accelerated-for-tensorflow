...
def gen_inference_batch(
    ...
    """
    Generator that generate batches with the following shape:
        data    = [batch_size, seq_length, -1]
        targets = [batch_size, seq_length, 1]
    """
    ...
    while count < rows:
        ...
        batch_data = batch_df.drop(['Is Fraud?'], axis=1).to_numpy().reshape(
...
def prepare_inference_data(batch_size: int, seq_length: int) \
        -> Generator[tuple[np.ndarray, np.ndarray]]:
    """
    Setup to get the inference data.
    Return the inference data and labels.
    ...
    csv_path = Path('./test_10k.csv')
    x_original = pd.read_csv(csv_path, index_col='Index')

    indices_path = Path('./test_10k.indices')
    test_indices = np.loadtxt(indices_path)
